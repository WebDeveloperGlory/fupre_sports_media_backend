...
exports.getAllSessions = async ({ isActive, year,  limit = 10, page = 1 }) => {
    ...
    // Find all sessions
    const allSessions = await db.TOTSSession.find( filter )
        .populate({
            path: 'competition',
            select: 'name'
        })
    ...
    const total = await db.TOTSSession.countDocuments(filter);
...
exports.getSingleSession = async ({ sessionId }) => {
    ...
    const session = await db.TOTSSession.findById( sessionId )
        .populate([
            {
                path: 'competition',
                select: 'name'
    ...
    // Count total votes
    const totalVotes = await db.Vote.countDocuments({ session: sessionId });
...
exports.getUserVote = async ({ userId }, { sessionId }) => {
    ...
    const userSubmittion = await db.Vote.findOne({ user: userId, session: sessionId })
        .populate([
            {
                path: 'session',
...
exports.getVoteCount = async ({ sessionId }) => {
    // Count total votes
    const totalSessionVotes = await db.Vote.countDocuments({ session: sessionId });

}

exports.getFinalVoteResult = async ({ sessionId }) => {
    // Check if result exists
    const result = await db.TOTSResult.findOne({ session: sessionId })
        .populate([
            {
                path: 'session',
                select: 'year competition isActive'
...
exports.createSession = async ({ year, competition, isActive, showVoteCount, startDate, endDate, adminVoteWeight }, { userId, auditInfo }) => {
    // Create session
    const createdSession = await db.TOTSSession.create({ year, competition, isActive, showVoteCount, startDate, endDate, adminVoteWeight });
...
exports.addPlayersToSession = async ({ sessionId }, { playerArray }, { userId, auditInfo }) => {
    ...
    const competition = await db.Competition.findById( session.competition );
    const competitionTeams = competition.teams.map( team => team.team.toString() );

    // Check if players are in competition
    const players = await db.Player.find({ _id: { $in: playerArray } });
    ...
    const updatedSession = await db.TOTSSession.findByIdAndUpdate(
        sessionId,
        {
            $addToSet: {
...
exports.toggleVote = async ({ sessionId }, { userId, auditInfo }) => {
    ...
    const udatedSession = await db.TOTSSession.findByIdAndUpdate(
        sessionId,
...
exports.submitAdminVote = async ({ sessionId }, { selectedFormation, selectedPlayers }, { userId, auditInfo }) => {
    ...
    const adminVote = await db.Vote.findOne({ user: userId, session: sessionId });
    ...
    // Create admin vote
    const createdAdminVote = await db.Vote.create({
        user: userId,
        session: sessionId,
        selectedFormation,
        selectedPlayers,
...
exports.submitUserVote = async ({ sessionId }, { selectedFormation, selectedPlayers }, { userId, auditInfo }) => {
    ...
    // Check if user vote exists
    const userVote = await db.Vote.findOne({ user: userId, session: sessionId });
    ...
    // Create user vote
    const createdUserVote = await db.Vote.create({
        user: userId,
        session: sessionId,
        selectedFormation,
        selectedPlayers
...
exports.calculateFinalResult = async ({ sessionId }, { userId, auditInfo }) => {
    ...
    // Check if result exists
    const foundResult = await db.TOTSResult.findOne({ session: sessionId });
    if (foundResult) return { success: false, message: 'Result Already Exists' };

    // Check if votes exist
    const votes = await db.Vote.find({ session: sessionId });
    ...
    const adminVote = await db.Vote.findOne({ user: session.adminVote, session: sessionId });
    ...
    const createdResult = await db.TOTSResult.create({
        session: sessionId,
